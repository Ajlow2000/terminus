...
My dad had the physical disc for our iMac that set in the family library.
...
I was/am a huge Star Wars fan and for a kid like, me a game that requires memorizing the names of various ships, their role in a fleet and what other ships they were strong and weak into was perfect.
...
To this day one of my favorite games to play.
...
The initial take-home project was interesting enough that I was curious about revisiting it and fixing some of the problems I created for myself with the first impl, but add in the idea of using this base to explore a space naval RTS game inspired by SW:EaW and now I was hooked.
...
- I currently am more motivated by building systems and lower level programming then I am with art and game design.
...
The simulator's arena will be a 2 dimensional space that tracks units that are capable of moving around the arena and engaging with each other.
...
As for the actual "game" itself-- lets just put a pin in that.
...
My first objective with this codebase is create some tooling to make generating unit tests a bit easier.
...
I actually implemented a first past at this tooling while writing this blog post and it looks like this:
...
This generates a normal rust unit test that I can manually add assertions for to match what behavior I expected to see (but didn't).
...
But if I ordered it from `posA` -> `posB`, and then ordered it `posC` before it arrived at `posB`, the unit would start orbiting around `posC` and never come to a rest.

// TODO - add video of the orbiting behavior

Now, this is a pretty basic simple bug that is easy to reason about and write a test for (literally the first feature implemented lol), but it's a good dress rehearsal for the record/replay/generate test loop.
